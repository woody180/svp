...
<section class="uk-section">
    <div class="uk-container min-height">
        ...
        <div>
            <form enctype='multipart/form-data' action="<?= baseUrl(urlSegments(1, true) . '/' . urlSegments(2, true)) ?>" class="uk-grid-match uk-child-width-1-1" method="POST" uk-grid>
                
                <?= setMethod('put') ?>
                <?= csrf_field() ?>
                
                <input type="hidden" value="<?= $article->thumbnail ?>" name="thumbnail_hidden">
                
                <div>
                    <label class="uk-form-label">სათაური</label>
                    <input type="text" class="uk-input uk-border-rounded" name="title" value="<?= $article->title ?>">
                ...
                <div>
                    ...
                    <select name="categories[]" class="svp-category-select uk-select uk-border-rounded" multiple>
                        ...
                        <option <?= array_search_index($usedCategories, 'id', $cat->id, false) ? 'selected' : '' ?> value="<?= $cat->id ?>"><?= trim($cat->title) ?></option>
                        <?php endforeach; ?>
                    </select>
                    ...
                    <div class="uk-grid-match uk-grid-small" uk-grid>
                        ...
                        <div class="uk-width-small">
                            <div class="uk-position-relative" data-style="border: 1px solid #e3e3e3; border-style: dashed; border-radius: 5px; overflow: hidden; height: 140px">
                                ...
                                <?php if ($article->thumbnail && !empty($article->thumbnail)): ?>
                                    <?= img(['src' => $article->thumbnail, 'class' => 'svp-thumbnail-review uk-display-block uk-object-cover uk-width-1-1'], true) ?>
                                <?php else: ?>
                ...
                <div>
                    ...
                    <textarea type="text" class="uk-textarea uk-border-rounded uk-height-small" name="description"><?= $article->description ?></textarea>
                    <?= show_error('errors', 'description') ?>
                </div>
                
                <div>
                    <label class="uk-form-label">სტატიის ტექსტი</label>
                    <textarea type="text" class="uk-textarea uk-border-rounded uk-height-small tinymce-editalbe-area" name="body"><?= relevantPath($article->body) ?></textarea>
